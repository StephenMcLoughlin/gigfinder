...
function friendReqHandler(action,reqid,user1,elem){
	...
	getElement(elem).innerHTML = "processing ...";
	var ajax = ajaxObj("POST", "php_parsers/friend_system.php");
	ajax.onreadystatechange = function() {
		if(ajaxReturn(ajax) == true) {
			if(ajax.responseText.trim() == "accept_ok"){
				getElement(elem).innerHTML = "<b>Request Accepted!</b><br />Your are now friends";
			} else if(ajax.responseText.trim() == "reject_ok"){
				getElement(elem).innerHTML = "<b>Request Rejected</b><br />You chose to reject friendship with this user";
			} else {
				getElement(elem).innerHTML = ajax.responseText;
			}
		}
	}
