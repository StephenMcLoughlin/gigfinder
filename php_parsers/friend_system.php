...
<?php
	if(isset($_POST['type']) && isset($_POST['user'])) {
		...
		if($_POST['type'] == "friend") {
			$sql = "SELECT COUNT(id) FROM friends WHERE user1='$user' AND accepted='1' OR user2='$user' AND accepted='1' LIMIT 1";
			$query = mysql_query($sql);
			...
			if($friend_count[0] > 99) {
				mysql_close();
				echo "$user currently has the max number of friends";
				exit();
			} else if($blockcount1[0] > 0) {
				mysql_close();
				echo "$user has you blocked";
				exit();
				...
				mysql_close();
				echo "You have a pending friend request already";
				exit();
			} else if($row_count4[0] > 0) {
				mysql_close();
				echo "$user has requested to friend with you";
				exit();
			} else {
			...
			$sql = "SELECT COUNT(id) FROM friends WHERE user1=$'luser' AND user2='$log_username' AND accepted='1' LIMIT 1";
			$query = mysql_query($sql);
			$row_count2 = mysql_fetch_row($query);
			...
			if($row_count[0] > 0) {
				...
				mysql_close();
				echo "No friendship could be found between your account and $user, therefore we can't unfriend you";
				exit();
			}
		}
	}
